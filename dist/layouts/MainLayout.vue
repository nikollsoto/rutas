        ...
        <transition name="fade">
          <q-card v-if="showNotifications" class="notification-panel shadow-5" role="dialog"
            ...
            <q-scroll-area class="notification-scroll-area">
              <q-list padding>
                <q-item v-for="note in notificationList" :key="note.id" clickable :class="{ 'unread': !note.read }">
            ...
            <div class="notification-footer row items-center justify-between q-pa-sm">
              <q-btn flat label="Limpiar todas" color="negative" @click="clearAll"
                :disable="notificationList.length === 0" />
...
// Importar imágenes desde src/assets para que Vite las procese en build
...
const unreadCount = computed(() => {
  let count = 0
  let index = 0
  while (index < notificationList.value.length) {
    if (notificationList.value[index] && !notificationList.value[index].read) {
      count++
    }
    index++
  }
...
function markAllAsRead() {
  ...
  let index = 0
  while (index < notificationList.value.length) {
    if (notificationList.value[index]) {
...
function deleteNotification(id) {
  ...
  let idx = -1
  let index = 0
  while (index < notificationList.value.length && idx === -1) {
    if (notificationList.value[index] && notificationList.value[index].id === id) {
      idx = index
    }
    index++
  }
