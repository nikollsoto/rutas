      ...
      <div class="col-12">
        <q-card class="shadow-1 hero-card q-mb-xl">
          <q-card-section class="flex flex-center">
            <PageHeader title="Parámetros de Seguridad y Control de Acceso" description="Ajusta roles, permisos y políticas de seguridad." align="center" />
          </q-card-section>
        </q-card>
      </div>
    </div>
          ...
          <div class="form-row">
            ...
            <div class="form-input-group">
              <q-checkbox
                ...
                color="green"
                :disable="loading"
                @update:model-value="updateForcePasswordChange"
              />
            </div>
          </div>
          ...
          <div class="form-row">
            ...
            <div class="form-input-group">
              <q-input
                ...
                @update:model-value="updateSessionTime"
                :min="1"
                :max="720"
              />
      ...
      <template #body>
        <div class="q-pa-md text-center">
          ...
          <p class="confirmation-text text-body1">
            ¿Desea guardar la configuración de <strong>SEGURIDAD</strong>?
          </p>
        </div>
      </template>
      ...
      <template #footer>
        <div class="row justify-end q-gutter-sm full-width">
          ...
          <Button
            ...
            custom-class="gradient-btn"
            :loading="loading"
            @click="submitSecurityConfiguration"
          />
        </div>
      </template>
    </Modal>
...
filtersData.value.category = 'SECURITY'

// ==================== CUSTOM API FUNCTIONS ====================

// Override fetchEntities to include category filter and process parameters
const fetchSecurityParameters = async () => {
  filtersData.value.category = 'SECURITY'
  await fetchEntities()
  
  // Process parameters from rows
  const data = rows.value
  const forcePasswordParam = data.find(param => param.name === 'CONTRASEÑA TEMPORAL INICIAL')
  const sessionTimeParam = data.find(param => param.name === 'TIEMPO DE SESION')
...
const submitSecurityConfiguration = async () => {
  loading.value = true
  try {
    const forcePasswordParam = rows.value.find(param => param.name === 'CONTRASEÑA TEMPORAL INICIAL')
    const sessionTimeParam = rows.value.find(param => param.name === 'TIEMPO DE SESION')

    const updates = []

    if (forcePasswordParam && forcePasswordParam.value !== forcePasswordChange.value) {
      updates.push(putData(endpoints.update(forcePasswordParam._id), { value: forcePasswordChange.value }))
    }

    if (sessionTimeParam && Number(sessionTimeParam.value) !== Number(sessionTime.value)) {
      updates.push(putData(endpoints.update(sessionTimeParam._id), { value: Number(sessionTime.value) }))
    }
